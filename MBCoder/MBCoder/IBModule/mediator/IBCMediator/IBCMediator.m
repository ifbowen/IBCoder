...
@interface IBCMediator ()

@property (nonatomic, strong) NSMutableDictionary *cachedTarget;

@end

@implementation IBCMediator

#pragma mark - public methods
+ (instancetype)sharedInstance
{
    static IBCMediator *mediator;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        mediator = [[IBCMediator alloc] init];
    });
    return mediator;
}
...
+ (IBCViewController *)enter_IBCViewController:(NSString *)vc params:(NSDictionary *)params {
    ...
    IBCViewController *ibc = [[cls alloc] init];
    ibc.params = params;
    return ibc;
}
...
- (id)performActionWithUrl:(NSString *)url completion:(void (^)(NSDictionary *))completion
{
    ...
    NSString *urlparams = [link query];
    for (NSString *param in [urlparams componentsSeparatedByString:@"&"]) {
    ...
    NSString *actionName = [link.path stringByReplacingOccurrencesOfString:@"/" withString:@""];
    if ([actionName hasPrefix:@"native"]) {
        return @(NO);
    }
    
    // 这个demo针对URL的路由处理非常简单，就只是取对应的target名字和method名字，但这已经足以应对绝大部份需求。如果需要拓展，可以在这个方法调用之前加入完整的路由逻辑
    id result = [self performTarget:link.host action:actionName params:params shouldCacheTarget:NO];
    if (completion) {
        if (result) {
