...
/*
 ...
 动画分类：
 序列帧动画，关键帧动画，补间动画，逐帧动画，属性动画，粒子动画，顶点动画
 
 序列帧动画：很多图片组合形成的动画
 关键帧动画：只需要给定几个关键帧的画面信息，关键帧与关键帧之间的过渡帧都将由计算机自动生成。
   补间动画：指的是计算机自动插值补全关键帧Keyframe之间的动画。所以Tween其实只是一个补全的过程，更加合理的称呼应该是关键帧动画。
   逐帧动画：一种在连续的关键帧中分解动画动作，即在时间轴的每一帧上绘制不同内容并使之连续播放成动画的一种常见的动画形式。骨骼动画
   顶点动画：所有让顶点随时间动起来的机制都能被称作顶点动画
   属性动画：围绕view属性做的动画。
