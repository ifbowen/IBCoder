...
/*
 ...
 2) Color Hits Green and Misses Red（光栅化）
    这个选项主要是检测我们是是否正确使用layer的shouldRasterize属性，shouldRasterize = YES开启光栅化。什么是光栅化？光栅化是将一个
 layer预先渲染成位图(bitmap)，再加入到缓存中，成功被缓存的layer会标注为绿色,没有成功缓存的会标注为红色，正确使用光栅化可以得到一定程度的
 性能提升。
 ...
 3）Color Copied Images（图片颜色格式）
    Shows images that are copied by Core Animation in blue苹果官方注释被拷贝给CPU进行转化的图片显示为绿色。那么这句话怎么理
 解呢？如果GPU不支持当前图片的颜色格式，那么就会将图片交给CPU预先进行格式转化，并且这张图片标记为蓝色。那么GPU支持什么格式呢？苹果的GPU只
 解析32bit的颜色格式，如果使用Color Copied Images去调试发现是蓝色，这个时候你也可以去找你们的UI眉眉了～
 
 知识扩展：
    32bit指的是图片颜色深度，用“位”来表示，用来表示显示颜色数量，例如一个图片支持256种颜色，那么就需要256个不同的值来表示不同的颜色，也
 就是从0到255，二进制表示就是从00000000到11111111，一共需要8位二进制数，所以颜色深度是8。通常32bit色彩中使用三个8bit分别表
 示R红G绿B蓝,还有一个8bit常用来表示透明度（Alpha）。
 
 Color Non-Standard Surface Formats (不标准的表面颜色格式)
 这个调试选项没有一篇博文讲过，都是直接略过，我也尝试很多途径去找这个选项到底是什么作用，然而苹果开发文档以及stack overflow都没有对这个有
 所解释。自己真机调试尝试了很多发现有个规律，就是打开这个选项，某些Label和Button的背景颜色都会出现银白色，但是不是必先现的，有些Label
 和Button依然正常颜色背景。其他ImageView等控件是不会出现银白色的背景颜色，猜想是不是和文本Text的设置有关系
 ...
 因为image size与imageView size不匹配，会消耗资源压缩图片。下图中的image实际size（81，110），顶部image正常，底部image出现黄色因
 为放在了一个size x 2的imageView容器中。
 ...
 7）Color Compositing Fast-Path Blue (快速路径)
    Places a blue overlay over content that is detached from the compositor.标记由硬件绘制的路径为蓝色，蓝色越多越好，可以
 对直接使用OpenGL绘制的图层进行高亮。没有对OpenGL有过多的研究，所以这里没办法给出demo，大家只需要记住蓝色越多越好就ok。
 
 8）Flash Updated Regions (重绘区域)
    Colors regions on your iOS device in yellow when those regions are updated by the graphics processor.这个选项会
 对重绘的内容高亮成黄色,重绘就是指使用Core Graphics绘制，绘制会损耗一定的性能，因此重绘区域应该越小越好。下图是用真机进入原生地图开启
 Flash Updated Regions 调试的效果图，很可惜截屏不能截到黄色的区域，因此我用红框圈起来，一共两处，坐上角的是在不停的刷新页面，右下角是在
 不停的刷新当前位置，因此都是使用Core Graphics重绘刷新的一种场景，并且你可以发现黄色区域很小，区域越小性能越好。
